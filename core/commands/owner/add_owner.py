...
def init(update,context):
    if update.message.reply_to_message:
        ...
        if row:
            message(update,context, "{} This owner already exists in the database".format(username))
        else:
            data = [(user_id, username)]
            UserRepository().add_owner(data)
            message(update,context, "You have entered {} [<code>{}</code>] a new owner in the database!".format(username,user_id))
    else:
